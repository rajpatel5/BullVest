 ...
 */
public final class WatchlistQueryUtils {

    /** Tag for the log messages */
    private static final String TAG = "WatchlistQueryUtils";

    /**
     * Create a private constructor because no one
     * should ever create a {@link WatchlistQueryUtils} object.
     */
    private WatchlistQueryUtils() {
    }

    /**
     * Query the IEX data set and return an array of ContentValues.
     */
    public static ContentValues[] fetchStockData(String requestUrl) {
        // Create URL object
        ...
        ContentValues[] stocks = extractFeatureFromJson(jsonResponse);
     ...
     */
    private static ContentValues[] extractFeatureFromJson(String stockBatchJSONStr){
        ...
        try {
            ...
            stockContentValues = new ContentValues[stockBatchJson.length()];

            for (int i = 0; i < stockBatchJson.length(); i++) {
                JSONObject stockJson = stockBatchJson.getJSONObject(NetworkUtils.allWatchlistSymbols.get(i));
                ...
                String companyName = quoteJson.getString(Constants.IEX_COMPANY_NAME);
                ...
                stockValues.put(StockContract.WatchlistEntry.COLUMN_SYMBOL, symbol);
                stockValues.put(StockContract.WatchlistEntry.COLUMN_COMPANY_NAME, companyName);
                stockValues.put(StockContract.WatchlistEntry.COLUMN_PRICE, price);
                stockValues.put(StockContract.WatchlistEntry.COLUMN_PRICE_CHANGE, priceChange);
                stockValues.put(StockContract.WatchlistEntry.COLUMN_PERCENT_CHANGE, percentChange);
